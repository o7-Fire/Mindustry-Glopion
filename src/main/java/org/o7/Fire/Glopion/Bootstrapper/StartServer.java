...
public class StartServer {
    ...
    public static Process run() throws IOException {
        File server = SharedBootstrapper.getMindustryFile(SharedBootstrapper.MindustryType.Server);
        if(!server.exists())
            SharedBootstrapper.download(SharedBootstrapper.getMindustryURL(SharedBootstrapper.MindustryType.Server), server);
